...
def test_cdc(neo4j_info):
    ...
    count = cdc.total_captured
    print("\tRetrieved {} event changes".format(count))
    total_captured, total_time, first_event_size = cdc.wait_for_results()
...
def test_make_changes(neo4j_info, target_change_rate, test_duration):
  cm = change_maker.changer(neo4j_info)
  cm.start(test_duration, target_change_rate)
  time.sleep(test_duration)
  total_changes, total_runtime = cm.wait_for_results()
